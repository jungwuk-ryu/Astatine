...
public final class AttributeInstanceArrayMap implements Map<Holder<Attribute>, AttributeInstance>, Cloneable {
    private static final int VANILLA_ATTRIBUTE_SIZE = Math.max(BuiltInRegistries.ATTRIBUTE.size(), 35); // DivineMC - Multithreaded Tracker
    ...
    private void ensureCapacity(int index) {
        if (index >= this.a.length) {
            this.a = Arrays.copyOf(this.a, index + 1);
        }
    }
    ...
    public AttributeInstance get(Object key) {
        return key instanceof Holder<?> holder && holder.value() instanceof Attribute attribute ? this.getInstance(attribute.uid) : null;
    }
